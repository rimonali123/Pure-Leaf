...
const AddressForm = () => {
    ...
    const onSubmit = async (data) => {
        ...
        const userInfoData = await axiosSecure.patch(`/userInfo/${user?.email}`, userInfo)
        // .then(res => {
        //     console.log('successfully send bio to database........', res)
        // })
        console.log('successfully send bio to database........', userInfo)
        if (userInfoData.data.modifiedCount > 0) {
            Swal.fire({
                position: "center",
                icon: "success",
    ...
    return (
        <div className="p-4">
            <h2 className="text-2xl font-semibold"> Billing & Shipping Address</h2>
